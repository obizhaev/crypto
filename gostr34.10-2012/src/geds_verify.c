...
int
geds_verify(const geds_context *ctx, char *msg, int n, const geds_point *Q)
{
	ec_point C, tmp;
	...
	rc = mpl_set_uchar(&r, ctx->s, ctx->length/2);
	if (rc != MPL_OK) {
		rc = GEDS_ERR;
		goto err_point;
	}
	...
	rc = mpl_set_uchar(&s, ctx->s + (ctx->length/2), ctx->length/2);
	if (rc != MPL_OK) {
		rc = GEDS_ERR;
		goto err_point;
	}
